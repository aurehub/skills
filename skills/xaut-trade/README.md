...
```bash
bash "$(p=$(cat ~/.aurehub/.setup_path 2>/dev/null); [ -f "$p" ] && echo "$p" || for _p in "$HOME/.claude/skills/xaut-trade/scripts/setup.sh" "$HOME/.aurehub/.agents/skills/xaut-trade/scripts/setup.sh" "$HOME/.agents/skills/xaut-trade/scripts/setup.sh"; do [ -f "$_p" ] && echo "$_p" && break; done)"
```
